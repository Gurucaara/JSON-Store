...
router.get("/walmart/:id", (req, res) => {
  const walmartId = parseInt(req.params.id);
  const singleWalmart = walmartData.find((item) => item._id === walmartId);

  if (!walmartId) {
    return res.status(404).json({
      message: " Walmart data was not found",
    });
  }
  res.json(singleWalmart);
});
