...
router.get("/phoneCase/:id", (req, res) => {
  const phoneCaseId = parseInt(req.params.id);
  const singlePhoneCase = phonecaseData.find(
    (item) => item._id === phoneCaseId
  );

  if (!phoneCaseId) {
    return res.status(404).json({
      message: " Phone Case  was not found",
    });
  }
  res.json(singlePhoneCase);
});
